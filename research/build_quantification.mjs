...
const studyParser = createStudyParser();
const mathAssets = embeddedMathAssets();
...
markdown += `本正文引用 ${sources.length} 项来源记录；同一项目的论文、仓库和规范可能分别登记，因此来源数与独立项目数分别统计。完整采集表另含仅供发现与继续阅读的记录。\n\n`;
...
renderer.paragraph = function(token) {
  ...
  if (token.tokens.length === 1 && token.tokens[0].type === 'image') return `${body}\n`;
  const lead = token.tokens.length === 1 && token.tokens[0].type === 'strong';
  return `<p${lead ? ' class="entry-title"' : ''}>${body}</p>\n`;
};
const tokens = studyParser.lexer(markdown);
const mathCounts = { display: 0, inline: 0 };
studyParser.walkTokens(tokens, token => {
  if (token.type === 'paragraph' && /^\s*\|[^\n]+\|\s*$/m.test(token.text)) throw Error(`Unrendered table: ${token.text.slice(0, 90)}`);
  if (token.type === 'text' && !token.tokens && token.text.includes('**')) throw Error(`Unrendered emphasis: ${token.text.slice(0, 100)}`);
  if (token.type === 'displayMath') mathCounts.display++;
  if (token.type === 'inlineMath') mathCounts.inline++;
});
studyParser.use({ renderer });
let body = studyParser.parser(tokens) + '</section>';
body = body.replace(/<table>([\s\S]*?)<\/table>/g, (_, content) => {
  const fieldTable = /<td>F\d{2}<\/td>/.test(content);
  return `<div class="table-scroll" tabindex="0"><table${fieldTable ? ' class="field-codebook"' : ''}>${content}</table></div>`;
});
...
<title>把 AI Harness 变成可比较的数据｜方法研究</title><script>${theme}</script>
<!-- KaTeX styles and fonts: ${mathAssets.license.replaceAll('--', '—')} -->
<style>${mathAssets.css}\n${css}</style></head>
<body><a class="skip" href="#content">跳到正文</a>
<button id="toggle-nav" aria-expanded="false" aria-controls="study-sidebar">☰ 章节目录</button>
<aside class="sidebar" id="study-sidebar"><a class="brand" href="../../index.html">HARNESS / 方法研究</a><p class="sidebar-note">用可核验的证据解释工具差异</p>
...
<footer>ProgrammerAsahi · 正文与原创图示 <a href="https://creativecommons.org/licenses/by/4.0/">CC BY 4.0</a>；程序代码 MIT。第三方材料保留原有权利。<br><a href="https://github.com/ProgrammerAsahi/ai-harness-comparison">项目仓库</a> · <a href="README.md">复算与维护说明</a> · <a href="../../THIRD_PARTY_NOTICES.md">数学排版与字体许可</a></footer></main>
...
const manifest = {
  ...
  mathematics: { ...mathAssets.manifest, ...mathCounts },
