...
b=title('Step Code：Pi 派生核心与 Step 产品能力')
b+=box(325,62,300,64,['终端界面／脚本输入'])
b+=box(325,174,300,84,['Pi 派生会话与代理循环','输入、工具反馈、继续与结束'],True)
b+=arrow('M475 126 V172')
b+=box(30,172,250,88,['Step 模型适配','动态目录 → 云端模型'])
b+=arrow('M325 206 H282')+arrow('M280 238 H323')
b+=box(670,172,250,88,['本地状态','历史、分支、摘要、目标'])
b+=arrow('M625 206 H668')+arrow('M670 238 H627')
b+=box(325,320,300,76,['Step 工具接口','权限模式与命令检查'],True)
b+=arrow('M475 258 V318')
b+=box(30,322,250,72,['子代理','独立上下文／可选工作树'])
b+=arrow('M325 246 H300 V300 H155 V320')
b+=box(30,454,250,72,['可选 workflow 编排','依赖可用的原生脚本环境'])
b+=arrow('M155 454 V396')
b+=box(325,454,300,72,['本机文件与命令','MCP／静态发布插件'])
b+=arrow('M475 396 V452')
b+=box(670,340,250,136,['按安装方式核对能力','审批 ≠ 系统沙箱','对话分支 ≠ 文件回滚'])
b+='<text class="small" x="31" y="569">主线沿用会话核心；产品层加入配置与工作方式。模型输入、工具执行和发布有各自的数据路径。</text>'
(D/'06-step-code.svg').write_text(frame('Step Code 架构与边界',950,595,b))
print('Authored 6 SVG diagrams')
