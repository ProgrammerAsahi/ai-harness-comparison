...
(() => {
  ...
  function revealInNav(link) {
    ...
    const viewportCenter = container.top + nav.clientTop + nav.clientHeight / 2;
    const itemCenter = item.top + item.height / 2;
    // Never use scrollIntoView: it can also move the document being read.
    // Center multiline titles too, stopping naturally at either scroll boundary.
    const target = nav.scrollTop + itemCenter - viewportCenter;
    nav.scrollTop = Math.max(0, Math.min(target, nav.scrollHeight - nav.clientHeight));
  }

  function update() {
    frame = 0;
    const shouldReveal = layoutDirty || revealRequested;
    ...
    if (active && !search.value.trim() && !active.group.hidden && !active.link.hidden) {
      ...
      if (changed || opened || shouldReveal) revealInNav(active.link);
    }
  ...
  // Manual navigation scrolling remains free; resuming article scrolling recenters it.
  window.addEventListener('scroll', () => schedule({ reveal: true }), { passive: true });
