# 06｜模型搭配与成本：按任务选择组合，核算实际投入

模型目录变化很快。以下型号依据已收集的官方资料，采集时间见来源记录；这些型号用于说明候选路线，实际配置时应结合来源日期核对可用性。产品界面、地区、账户和 Provider 都可能限制可用型号。
...
|---|---|---|---|
| OpenAI：GPT-6 Astra、Sol、Luna；过渡期仍提供 GPT-5.6 系列 | Codex；已明确支持相应型号的多模型产品 | Astra 复杂长任务；Sol 日常开发与多步工作；Luna 清楚、重复且易验收的任务 | GPT-6 Sol／Luna 正在推出；以账户和入口实际目录为准，不能由原生支持推断第三方已适配 |
...
官方目录已加入 GPT-6 Sol 和 Luna，并说明推出期间 GPT-5.6 系列仍可用。场景表中的 GPT-5.6 组合可作既有环境的对照；新用户先核对当前目录，再用同一任务比较新旧候选的效果与成本。[官方模型目录](https://learn.chatgpt.com/docs/models.md)
...
4. **错误恢复。** 文件不存在、命令失败、工具参数不合法时，能否利用错误信息调整后续动作。
5. **历史与压缩。** 会话变长或恢复后，是否记得关键约束；工具调用与结果有没有失配。
6. **控制与成本。** 中断能否生效；子任务是否继续烧费；模型窗口、输出上限与 Harness 的估算是否一致。

某些模型需要保留推理状态、签名或特定响应字段。网关如果只转发文本，看起来仍能聊天，却可能破坏长任务。也有服务仅在协议表面兼容，实际不支持某些参数。遇到这种问题时，先检查适配层是否完整保留了多轮执行所需的信息。
...
Qoder 的 Repo Wiki 单独计费就是一个具体例子：主对话使用 BYOK，不代表产品里的所有模型能力都改走同一账户。Factory 文档又说明不同入口对 BYOK 的支持范围不同。判断能否使用自己的 Key 时，应逐项核对所需功能的认证与计费方式。[Qoder](https://docs.qoder.com/qoder/custom-models.md)、[Factory](https://docs.factory.ai/model-independence/byok.md)

### 缓存与压缩也有取舍

模型服务可能对重复输入提供缓存优惠。稳定的项目规则与工具列表有利于复用；频繁修改系统提示、切模型或更换工具集合，可能降低命中。另一方面，为了缓存而保留大量无关历史，也可能让任务更难理解。

压缩通常需要额外模型调用，且可能丢信息。压缩应以保留准确、足够的任务状态为目标，再在此基础上控制 Token 消耗。某次压缩少花了一点钱，却导致代理重做整个调查，整体并不划算。
...
对重要任务，可以安排第二个模型或独立代理审查。审查时提供原始材料、实际结果与验收条件，让它形成自己的判断，再比较两份结论。
